...
c   subroutine hpxhdr
c
      subroutine hpxhdr(rnxvsn, nsat_sys, csat_sys, nobs_typ, cobs_typ)
c
c=======================================================================
c     ****f* qualicontr/hpxhdr
c
c   FUNCTION   
c   
...
c     ***
c
C     $Id: hpxhdr.f,v 1.0 2009/06/28 $
c=======================================================================
c
      implicit none
c
...
c
c     write into clean file:102
c
      integer       nobs_typ1(MAX_SAT_SYS)
      character*3   cobs_typ1(MAX_SAT_SYS, MAX_OBS_TYP)
c
c     local variable
c
c     RINEX header
...
c
      if(ihdr.gt.40)then
        ...
        write(*,*) 'change the record line in hpxhdr.f'
        stop
      endif
c
...
c
         do i=1, nobs_typ(1)
            cobs_typ1(1,i) = cobs_typ(1,i)
         enddo
c
         nobs_typ1(1) = nobs_typ(1) + 2
c
         cobs_typ1(1,nobs_typ(1)+1) = 'Nw'
         cobs_typ1(1,nobs_typ(1)+2) = 'Sw'
c
         if(    nobs_typ1(1) .le. 9)then
c
c        read the first line of TYPES of OBSERV
         write(line(7:60),'(9(4X,A2))')(cobs_typ1(1,i),i=1,nobs_typ1(1))
c
         write(102,'(A80)') line
c
         elseif(nobs_typ1(1) .gt. 9 .and. nobs_typ1(1) .le. 18)then
c
c        read the first line of TYPES of OBSERV
         write(line(7:60),'(9(4X,A2))')(cobs_typ1(1,i),i=1,9)
c
         write(102,'(A80)') line
c
c        read the second line of TYPES of OBSERV
         write(line(7:60),'(9(4X,A2))')
     &        (cobs_typ1(1,i),i=10,nobs_typ1(1))
c
         write(102,'(A80)') line
c
         else
