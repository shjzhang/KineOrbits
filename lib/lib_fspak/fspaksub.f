...
c Ignacy MISZTAL
c Mauricio ELZO
c Juan Pablo SANCHEZ, May 21, 2007
c
c second
c count_nza
c checkb
...
c     -----------------
      subroutine s_ori (n,iu,ju,iju,u,d,b,s,tol)
c     -----------------
c triangular system iuju,iju,u times a vector (b), output go to s
c JPSS 05/23/2007
      implicit none
      integer n,iu(1),ju(1),iju(1),i,j,k
      real*8 u(1),d(1),b(1),s(1),bi,tol,a
...
c full forward
      do i=1,n
         ...
         do k=iu(i),iu(i+1)-1
            j=ju(iju(i)+k-iu(i))
            a=0
            if(d(i).gt.tol)a=-u(k)/sqrt(d(i))
            s(j)=s(j)+a*bi
         enddo
      enddo
      do i=1,n
       a=0
       if(d(i).gt.tol)a=b(i)/sqrt(d(i))
       s(i)=s(i)+a
      enddo 
      return
      end
...
c     -----------------
      subroutine s_inv (n,iu,ju,iju,u,d,b,tol)
c     -----------------
c lower triangular system iuju,iju,u solve using b as RHS, output is the same b
c updated
c JPSS 05/23/2007
      implicit none
      integer n,iu(1),ju(1),iju(1),i,j,k
      real*8 u(1),d(1),b(1),bi,tol
...
c     ------------------
      subroutine assignation (n,vec1,vec2)
c     ------------------
c permutes vector vec1 according to permutation vector p into vec2
c M_P-E Wed Jul 1, 1992 11:12:01
      integer n,i
