...
c Metis modifications by Karin Meyer (2005) 
c    incorporated by I. Misztal (Sept 1, 2005)
c -----------------------------------------------------------
      implicit none
...
!     additions by km -> set metis=.false. to revert to genmmd
      logical :: metis=.true.
      integer, dimension(0:7) :: metis_opts
...
c        -------------------------------
! changes by km : add metis for re-ordering
         if( metis) then
            Print*,'Ordering by Metis 4.0 with options=0'
!           "default" for metis -> fastest, but can be improved upon
            metis_opts =0
!           something like this worked well for me; uncomment to try
!           metis_opts = (/ 1, 1, 2, 1, 0, 1, 0, 7 /) ! random edge matching,
                                                      ! 7 graph separators
            call METIS_NodeND ( n, is(xadj), is(adjncy), 1, metis_opts, 
     &                          is(perm), is(invp) )

!           metis does not return "maxju" which is the no. of subscripts
!           used; 
!           Here, it is set to the number of nonzeros in the input matrix. Then, it
!           is increased in the symbolic factorization (SMBFCT) until flag=0
            maxju=ia(n+1)-1
...
c     ------------------------------
       do           
! loop through symbolic factorization until memory requirements are determined
         iu     = invp   + n
         iju    = iu     + n+1
         ju     = iju    + n+1
         xadj   = ju     + maxju
