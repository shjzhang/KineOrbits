...
cz       INTEGER  LINE
cz       data line/0/
cz       INTEGER  NVS,NTARG,NCTR,NCOORD
cz       INTEGER  NPT
cz       data npt/100/
czcz%
cz       integer ios
cz       character*40 test_f
czcz%
czcz% initialization 
czcz% in order to avodi floating exception
cz       do i=1, 3
cz         ss(i)=0.0d0
cz       enddo
cz       do i=1, 6
...
cz       enddo
cz
czC      Write a fingerprint to the screen.
cz
cz       WRITE(*,*) ' JPL TEST-EPHEMERIS program. ' //
cz     .            ' Last modified July 1997.'
cz
czC      Print the ephemeris constants.
cz
cz       CALL  Const (NAMS, VALS, SS, NVS)
cz
cz       WRITE (*,'(/3F14.2/200(/2(A8,D24.16)))') SS,
cz     .                                         (NAMS(I),VALS(I),I=1,NVS)
cz
czcz%   open the file instead by the standard input 
cz      test_f = '../test_data/testpo.405'
cz      open(unit=444, file=test_f, status='old',iostat=ios)  
cz      if(ios.ne.0)then
cz        write(*,*) 'open test file error'
cz        stop
cz      endif
czC     Skip the file header comments.
cz
cz1     READ(444,'(a3)')ALF3
cz      IF(ALF3 .NE. 'EOT') GO TO 1
cz
cz      WRITE(*,'(A92)') 
cz     . '  line -- jed --   t#   c#   x#   --- jpl value ---'//
cz     . '   --- user value --    -- difference --'
cz      WRITE(*,*) 
cz
czC     Read a value from the test case; skip if not within the time-range
czC     of the present version of the ephemeris
cz
cz2     READ(444,'(15X,D10.1,3I3,D22.13)',END=9)  
cz     . ET,NTARG,NCTR,NCOORD,XI
czcz    
czcz      write(*,'(15X,f10.1,3I3,D22.13)')
czcz     & ET,NTARG,NCTR,NCOORD,XI
cz
czcz%      write(*,*) ET, SS(1), SS(2)
czcz%      pause
czCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
cz
czC  NOTE : Over the years, different versions of PLEPH have had a fifth argument:
czC  sometimes, an error return statement number; sometimes, a logical denoting
czC  whether or not the requested date is covered by the ephemeris.  We apologize
czC  for this inconsistency; in this version, we use only the four necessary 
czC  arguments and do the testing outside of the subroutine.
cz
cz      IF(ET.LT.SS(1))then
cz        GOTO 2
cz      endif
cz      IF(ET.GT.SS(2))then
cz        GOTO 2
cz      endif
cz
cz      CALL  Pleph ( ET, NTARG, NCTR, R )
cz
czCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
cz
cz      DEL=DABS(R(NCOORD)-XI)
cz      if(ntarg .eq. 15 .and. ncoord .eq. 3)
cz     * del=del/(0.23d0*(et-2451545.d0))
cz
cz      LINE=LINE+1
cz
cz      IF ( MOD(LINE,npt) .EQ. 0 )  WRITE(*,200)
cz     . LINE,ET,NTARG,NCTR,NCOORD,XI,R(NCOORD),DEL
cz 200  FORMAT(I6,F10.1,3I5,3F20.13)
cz
czC  Print out WARNING if difference greater than tolerance.
czC
cz      IF (DEL .GE. 1.D-13) WRITE(*,201) 
cz     . LINE,ET,NTARG,NCTR,NCOORD,XI,R(NCOORD),DEL
cz 201  FORMAT(/ '*****  WARNING : next difference >= 1.D-13  *****'//
cz     . I6,F10.1,3I3,3F20.13/' ')
cz
cz      GO TO 2
cz
cz   9  STOP
...
C        
     ...
     .  ((IPT(I,J),I=1,3),J=1,12),NUMDE,(IPT(I,13),I=1,3)
cz%
ccc     write(*,*) TTL,CNAM,SS,NCON,AU,EMRAT,
ccc  &  ((IPT(I,J),I=1,3),J=1,12),NUMDE,(IPT(I,13),I=1,3)
cz%        pause
C
