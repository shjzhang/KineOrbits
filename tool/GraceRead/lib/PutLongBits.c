...
static char SccsId[] = "$Id: PutLongBits.c,v 1.2 2004/08/30 21:03:35 wib Exp $";
...
/
/Input variables:
/   x  input long from which the bits are to be taken
/   px starting position of the bits, with 0 being the lsb.
/   n  number of bits to put.
/Output variables:
/   y  output long where the bits are put
...
/
/********************************************************************/
int            PutLongBits(unsigned long x, int px,
                   unsigned long *y, int py, int n)
{
   unsigned long   a, b, c;
