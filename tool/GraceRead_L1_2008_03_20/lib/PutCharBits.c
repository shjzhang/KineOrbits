...
static char SccsId[] = "$Id: PutCharBits.c,v 1.2 2004/08/30 21:03:35 wib Exp $";

/*******************************************************************
/Entry Point: PutCharBits.c
/
/Description:
/   This function takes n bits in the input variable starting
/   at position px, and puts them into the output variable starting
/   at position py.  The call is PutCharBits(x, px, &y, py, n).
/   For example, PutCharBits(x, 3, &y, 6, 2) puts
...
/
/Input variables:
/   x  input char from which the bits are to be taken
/   px starting position of the bits, with 0 being the lsb.
/   n  number of bits to put.
/Output variables:
/   y  output char where the bits are put
...
/
/********************************************************************/
int            PutCharBits(unsigned char x, int px,
                   unsigned char *y, int py, int n)
{
   unsigned char   a, b, c;
