...
c     MW combination
      tmp_Nw(tmp_nsat)   =
     &      (f1_GPS   *tmp_L1 - f2_GPS   *tmp_L2)/(f1_GPS   -f2_GPS   )
     &     -(f1_GPS   *tmp_P1 + f2_GPS   *tmp_P2)/(f1_GPS   +f2_GPS   )
c
...
c
c        write(201,'(F20.9,4F14.3)') time,(xrcv(k),k=1,3),crcv
c
c        the epoch time must be corrected to the truth time
c        
         write(*,*)  time, crcv/c_light

         write(201,'(F20.9,4F14.3)') 
     &   time-crcv/c_light,(xrcv(k),k=1,3),crcv
c        
c        information
c        ***********
c
         write(301,'(F20.9,I6    )') time,tmp_nsat
c
c        statis
c        ******
c   
         write(303,'(F20.9,6F8.3 )') time,sig,
...
c
c           if(flag.EQ.1)then
c           iAMB_iSAT(isat) = iAMB_iSAT(isat) + 1
c           endif
c
c           Namb_base = 0
c           do k=1,(isat-1)
c           Namb_base= Namb_base + NAMB_iSAT(k)
c           enddo
c
c           idx_iamb = 4*NEPO + Namb_base + iAMB_iSAT(isat) 
c
c**         ambiguity
c
c           N3       = x0(idx_iamb)
c           sig_N3   = sig*dsqrt( Qxx(idx_iamb) )
c
c           Nw       = tmp_Nw(i)
c
c           GPS position and velocity, clock error
c
c           call corr_trs(Time,iPRN,xrcv,crcv,xtrs,vtrs,ctrs)
c
c**         elvation
c
c           call x_elv(xrcv,xtrs,elv)
c
c**         residual
c
c           if(    observ_model.EQ.1 .or. observ_model.EQ.3)then
c
c              rP3      = L( RL( (iobs-1)*2+1) )
c              rL3      = L( RL( (iobs-1)*2+2) )
c
c              NOTES: Nw, N3 with unit: meters,i.e. Nw = lam*phi
c
c              write(301,'(I3,2F8.3,I4,4F8.3)') 
c    &               iPRN,rP3,rL3,flag,Nw,N3,sig_N3,elv*180.0d0/pi
c
c           elseif(observ_model.EQ.2 .or. observ_model.EQ.4)then
c
c              rP3      = 0.0d0
c              rL3      = L( RL( iobs ) )
c
...
c
c           else
c              write(*,*) 'SmartPPP/x_wrt'
c              write(*,*) 'please extend for this model'
c              stop
c           endif
c
c        enddo
