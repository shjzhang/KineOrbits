...
c
      ...
      enddo
c
      if(dabs(time-time_fore).GE.360.0d0)then
c
         time_JD = sec2jd(time)
...
c        write(*,*) time_JD, (sun_state_icrs(i),i=1,3)
c
c        Last Modified 2010/11/23
c        sun state from planet_state have unit of km
c
         sun_state_icrs(1) = sun_state_icrs(1)
         sun_state_icrs(2) = sun_state_icrs(2)
         sun_state_icrs(3) = sun_state_icrs(3)
c
...
c
      ...
      z     = xtrs(3)
c
c     Modified 2010/11/23
c     returned sun state has the unit of kilometers
      x_sun = sun_state_itrs(1)*1000.0d0
      y_sun = sun_state_itrs(2)*1000.0d0
      z_sun = sun_state_itrs(3)*1000.0d0
c
...
c     write(*,*) iPRN, trs_pco(iPRN,1), trs_pco(iPRN,2), trs_pco(iPRN,3)
c
      do i=1,3
c
