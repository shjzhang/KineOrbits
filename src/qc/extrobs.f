...
c
     ...
     &                   raw_epo, raw_obs, raw_lli, raw_snr,
...
c
c        if no P1 types in observation header
         if(    idx_P1.NE.0)then
            P1(k) = raw_obs(i, idx_P1)
         elseif(idx_C1.NE.0)then
            P1(k) = raw_obs(i, idx_C1)
         endif
c        if P1.EQ.0.0d0 but C1 .NE. 0.0d0
         if(dabs( P1(k)-0.0d0 ).lt.1.0D-10)then
            P1(k) = C1(k)
         endif
c
