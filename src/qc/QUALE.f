...
c   USAGE
c
c     QUALE  -i  input_file  -o  output_file    
c
C   ARGUMENTS
C
...
c
      character*80  line
      character*200 file_arc     
      character*200 file_outlier 
      character*200 file_summary 
c
c     loop
c
      integer       i, j, k
...
c
      QUALICONTR_CONF = trim(HOPES_HOME)//trim(QUALICONTR_CONF)
c
c     parse the parameters in QUALE.conf
c
...
c     open(unit=211,file='211.txt',status='replace',iostat=ios)
c
      if(ios.NE.0)then
         write(*,*) 'QUALE/QUALE.f'
...
c     open(unit=411,file='411.txt',status='replace',iostat=ios)
c
      if(ios.NE.0)then
         write(*,*) 'QUALE/QUALE.f'
         write(*,*) '  open lvl2 files error!'
...
c
      if(len_trim(file_summary).gt.200)then
         write(*,*) 'QUALE.f'
         write(*,*) 'Length of the rinex summary file name is more than'
         write(*,*) '200'
         stop
      endif
c
...
c
         call qualelib(nrec,iPRN,EPOCH,L1,L2,P1,P2,S1,S2,
     &                                 Nw,L4,P4,L3,P3,Lw,
     &                                 Pw,iflag)
c
