...
c
 1000       format(I6, F20.9, (X,I3), 11(F14.3,I1,I1))
c
         else
c
...
c
      if(lvl1_LLI(irec,idx_L1).EQ.9)then
         goto 100
      endif
c
...
c
      tmp_time            = lvl1_Time(irec)
      tmp_iPRN(tmp_nsat)  = lvl1_iPRN(irec)
      iPRN                = lvl1_iPRN(irec)
c
      if(    iPRN.GE.000.and.iPRN.LT.100)then
         write(tmp_cPRN(tmp_nsat),'(I3.2)') iPRN
         tmp_cPRN(tmp_nsat)(1:1) = 'G'
      elseif(iPRN.GE.100.and.iPRN.LT.200)then
         iPRN = iPRN - 100
         write(tmp_cPRN(tmp_nsat),'(I3.2)') iPRN
         tmp_cPRN(tmp_nsat)(1:1) = 'E'
      elseif(iPRN.GE.200.and.iPRN.LT.300)then
         iPRN = iPRN - 200
         write(tmp_cPRN(tmp_nsat),'(I3.2)') iPRN
         tmp_cPRN(tmp_nsat)(1:1) = 'R'
      else
...
c
      tmp_flag(tmp_nsat)  = lvl1_LLI (irec,idx_L1)
c
      if(tmp_flag_fore(iPRN).EQ.1.and.tmp_flag(tmp_nsat).NE.9)then
         tmp_flag     (tmp_nsat) = tmp_flag_fore(iPRN)
         tmp_flag_fore(iPRN)      = 0
      endif
c
cZ    if(tmp_flag(tmp_nsat).NE.9)then
cZ       tmp_nsat1 = tmp_nsat1 + 1
cZ    endif
c
      do k=1, nobs_typ(1)
         tmp_OBS(tmp_nsat, k)= lvl1_OBS(irec, k)
         tmp_LLI(tmp_nsat, k)= lvl1_LLI(irec, k)
         tmp_SNR(tmp_nsat, k)= lvl1_SNR(irec, k)
      enddo
c
...
c
      if(    tmp_nsat.lt.5)then
c
c        write observables block header(time and satellie number)
c
         write(*,'(F20.9,x,I3)') tmp_time_fore,tmp_nsat
c
c        write observables block header(time and satellie number)
c
CZ       write(102, fmt=2000) tmp_time_fore, tmp_nsat
c
         do i=1, tmp_nsat
c
...
c
            tmp_LLI(i,idx_L1) = 9
c           
CZ          write(102, fmt=3000) 
CZ   &      tmp_cPRN(i),(tmp_OBS(i,k),tmp_LLI(i,k),
CZ   &                                tmp_SNR(i,k), k=1,nobs_typ(1))
c
         enddo
c
      elseif(tmp_nsat.ge.5)then
c
c        write observables block header(time and satellie number)
c
         write(102, fmt=2000) tmp_time_fore, tmp_nsat
c
c        write observables 
c
         do i=1, tmp_nsat
