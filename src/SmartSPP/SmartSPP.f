...
c     tropospheric will not be estimated
c   
c   Notes
c
...
c
         write(*,*)
         write(*,*)'Usages: SmartSPP -ihpx f -isp3 f -iclk f [-iatt f]',
     &             '-o f'
c
         write(*,*)
         write(*,*)'Arguments:'
...
c
c     write(*,'(  (x,A)   )') 
c    &     'Are the above parameters right?[y/n]'
c     write(*,'(  (x,A)   )') 
c
c     read(*,*) yes
c
c     if(trim(yes).NE.'y'.and.trim(yes).NE.'yes')then
c        write(*,*) 'Users should change the parameters!'
c        stop
c     endif
c
c     read SP3
...
c
      write(*,*) 'ppos'
c
      call ppos(rnxvsn,nsat_sys,csat_sys,nobs_typ,cobs_typ)
c
c     close file
c     ==========
c
