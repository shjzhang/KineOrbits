...
c
      ...
      z     = xtrs(3)
c
      x_sun = sun_state_itrs(1)*10**3 
      y_sun = sun_state_itrs(2)*10**3
      z_sun = sun_state_itrs(3)*10**3
c
...
c
      write(*,*)'trs_pco'
      write(*,*) time, iPRN, 
     &      trs_pco(iPRN,1), trs_pco(iPRN,2), trs_pco(iPRN,3),
     &ex(i)*trs_pco(iPRN,1)+ey(i)*trs_pco(iPRN,2)+ez(i)*trs_pco(iPRN,3)
